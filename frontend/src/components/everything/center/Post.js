...
import SinglePageContext from '../../../pages/singlePost/SinglePageContext';
...
export default function Post({post,user, LogedInUserLikesPosts}) {
  ...
  useEffect(()=>{
    LikepostCounterStopinggoingtoZero()
      // eslint-disable-next-line react-hooks/exhaustive-deps
},[isActive])



  const LikepostCounterStopinggoingtoZero = () => {
    if (post?.likes?.length === 0 &&
       isActive === true) 
    {
      setLikesCounter(post?.likes?.length + 1)
    }
 }
      ...
      <div id="postBox" >
       ...
       <Link to={`/profile/${post?.user?.username}`} >  <img src={post?.user?.picture} alt="" /></Link>   
        ...
        <div id="mainpost" >
          ...
          <div id="postPara"> <p  onClick={savethepageview}><Linkify  options={{target:'blank'}}>{post.text}</Linkify></p>
          {
